...
#################################### INSERT URL ####################################
    ...
    for link in links:
        ...
        try:
            pre_pos_and_company = soup.find(class_='profile-topcard__previous-positions flex mt3').text.split()
        except:
            pre_pos_and_company = []
        ins = ''
        for k in range(int(len(pre_pos_and_company)/2)):
            ins += pre_pos_and_company[2*k] + ' ' + pre_pos_and_company[2*k+1] + ';'
        pre_pos_and_company = ins
        #print('**** {} ****'.format(pos_and_company))
        #index_at = pos_and_company.index('at')
        #position = ' '.join(pos_and_company[:index_at])
        #company = pos_and_company[index_at + 1]
        try:
            edu1 = soup.find(class_='profile-education__school-name t-16 t-16--open t-bold t-black').text.split()
        except:
            edu1 = []
        try:
            edu2 = soup.find(class_='profile-education__degree inline t-14 t-black mb1').text.split()
        except:
            edu2 = []
        try:
            edu3 = soup.find(class_='profile-education__field-of-study inline t-14 t-black').text.split()
        except:
            edu3 = []
        try:
            edu4 = soup.find(class_='profile-education__dates t-12 t-black--light').text.split()
        except:
            edu4 = []
        education = ' '.join(edu1)+';'+' '.join(edu2)+';'+' '.join(edu3)+';'+' '.join(edu4)
        ...
        person_profile['previous'] = pre_pos_and_company
        person_profile['education'] = education
...
def scrap_from_keywords(args, driver,keywords=None):
    #allcookies = driver.session_id#get_cookies()
    #print(allcookies)
    ...
    filter_page_url = 'https://www.linkedin.com/sales/search/people?doFetchHeroCard=true&logHistory=true&rsLogId=1382593556&searchSessionId=1qpyQattToCh2lFvET1Cwg%3D%3D'
    #p = filter_page_url.split('&')#&keywords=speech%20recognition
    words = filter_page_url + '&keywords=' + '%20'.join(keywords.split(' '))
    driver.get(words)
    time.sleep(3)

    #kwd_input = driver.find_element_by_id('bpr-guid-1612009') #017('ember44-input')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--search_words', type=str, help='search keywords')
    parser.add_argument('--url', type=str, default=None)
    parser.add_argument('--mode', type=str, default='url')
    parser.add_argument('--email', type=str, help='sales natigator email')
    ...
    username.send_keys(args.email)  # Insert your e-mail

    password = driver.find_element_by_id('session_password')
    password.send_keys(args.pwd)  # Insert your password here
